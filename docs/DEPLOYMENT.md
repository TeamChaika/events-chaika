...
До подтверждённой оплаты реальные SMS с билетами не отправляются. После покупки проверить статус `paid`, выпуск билетов, задания Telegram и SMS в очереди. Только `delivered` подтверждает доставку SMS со стороны провайдера; получение на телефоне проверяет покупатель. Отдельный тест Telegram и создание QR не заменяют эту проверку.
