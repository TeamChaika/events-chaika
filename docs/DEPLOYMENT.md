...
Прямая отправка Telegram из Apps не прошла сетевую проверку. На выбранном сервере «Отчеты» подготовлен отдельный `chaika-events-telegram-proxy.service`: TLS CONNECT на порту 55434, обязательная авторизация, единственный разрешённый адрес `api.telegram.org:443`, выход к Telegram через IPv6. Содержимое сообщений остаётся внутри TLS-соединения приложения с Telegram. Оплата и SMS не используют proxy.
