...
Если браузер запрещает автозапуск или видео не начало играть за 12 секунд, остаётся постер текущего ролика и кнопка «Смотреть приглашение». Нажатие вызывает `video.play()` непосредственно в обработчике жеста. При `prefers-reduced-motion` видео также запускается только кнопкой. Ошибка медиа открывает сам билет; старый CSS-ролик больше не подставляется. Ссылка «Открыть билет» остаётся доступной во время загрузки и воспроизведения.
